    ...
    let result = contents
        ...
        .filter_map(|line| {
            if regex.is_match(line) {
                Some(regex.replace(line, substitution))
            } else {
                None
            }
        })
...
fn main()
{
    ...
    let mut out_file =
        fs::File::create(&libalan_ll_path).expect("Failed to create the output file");
    ...
    //make bitcode file;
    let _cmd = cmd::new("llvm-as")
        .arg("-o")
        .arg(bit_file_path)
        .arg(libalan_ll_path)
        .output()
        .expect("Couldn't generate libalan.bc");
}
